...
switch (platform) {
    ...
    switch (arch) {
      case "arm64":
        localFileExisted = existsSync(
          join(__dirname, "blazu.android-arm64.node")
        );
        try {
          if (localFileExisted) {
      ...
      case "arm":
        localFileExisted = existsSync(
          join(__dirname, "blazu.android-arm-eabi.node")
        );
        try {
          if (localFileExisted) {
  ...
  case "darwin":
    localFileExisted = existsSync(
      join(__dirname, "blazu.darwin-universal.node")
    );
    try {
      if (localFileExisted) {
...
const { serialize_json } = nativeBinding;

module.exports.serialize_json = serialize_json;
